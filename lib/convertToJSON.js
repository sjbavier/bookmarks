...
module.exports = function(file){
  ...
  var fs = require('fs');
  var stopWords = require('./removeStopWords');
  ...
  function createFile(fileString) {
    ...
    $('a').each(function(i, elem) {
      ...
      title = title.replace(/[^A-Za-z\d\s.]/ig, "").toLowerCase(); 
  ...
  function frequency(titles, marks) {
    ...
    var wordsArray = titlesString.split(' ');
    var wordsArrayFiltered = wordsArray.removeStopWords();
    var sortable = [];
  ...
  function categorize(sortable, marks) {
    for (var x = 0; x < sortable.length; x++) {
      for (var y = 0; y < marks.length; y++) {
        if (marks[y]['title'].toLowerCase().replace(/(-|,|\/)/ig, ' ').split(' ').indexOf(sortable[x][0]) != -1) {
